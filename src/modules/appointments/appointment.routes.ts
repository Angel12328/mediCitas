...
async function loadScopedAppointment(userId, roles, appointmentId) {
  ...
  const isScheduleDoctor =
    roles.includes('DOCTOR') && appointment.schedule.doctor.employee.user?.id === userId;
...
export async function appointmentRoutes(app) {
  ...
  app.post(
    '/',
    { preHandler: [authenticate, validate({ body: bookAppointmentSchema })] },
    async (request, reply) => {
  ...
  app.get(
    '/',
    { preHandler: [authenticate, validate({ query: appointmentsQuerySchema })] },
    async (request) => {
      const user = request.user;
  ...
  app.patch(
    ...
    async (request) => {
      ...
      if (!canTransition(dto.status, nextStatus, isOwner, isStaff)) {
        throw new AppError(
          'CONFLICT',
          `Transición inválida de ${dto.status} a ${nextStatus} para tu rol`,
        );
      }
