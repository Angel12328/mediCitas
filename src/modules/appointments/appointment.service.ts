...
export const OCCUPYING_STATUSES = ['PENDING', 'CONFIRMED', 'COMPLETED'] as const;
...
export async function bookAppointment(input: {
  ...
  if (hoursUntil < MIN_ADVANCE_HOURS) {
    throw new AppError(
      'INVALID_ADVANCE',
      `No se puede agendar con menos de ${MIN_ADVANCE_HOURS} horas de antelación`,
    );
  }
  ...
  if (daysUntil > MAX_ADVANCE_DAYS) {
    throw new AppError(
      'INVALID_ADVANCE',
      `No se puede agendar con más de ${MAX_ADVANCE_DAYS} días de antelación`,
    );
  }
  ...
  const attemptBooking = async () => {
    return prisma.$transaction(async (tx) => {
      ...
      if (!schedule) throw new AppError('NOT_FOUND', 'Horario no encontrado o inactivo');
      if (!maskIncludesDay(schedule.daysBitmask, input.date)) {
        throw new AppError(
          'VALIDATION_ERROR',
          'El horario no atiende el día correspondiente a la fecha solicitada',
        );
      }
      ...
      if (duplicate) {
        throw new AppError(
          ...
          'Ya tienes una cita reservada en este horario para esa fecha',
        );
      }
      ...
      if (dailyCount >= MAX_DAILY_APPOINTMENTS) {
        throw new AppError(
          ...
          `Máximo ${MAX_DAILY_APPOINTMENTS} cita(s) por día. Contacte a recepción para excepción`,
        );
      }
      if (weeklyCount >= MAX_WEEKLY_APPOINTMENTS) {
        throw new AppError(
          'WEEKLY_LIMIT_EXCEEDED',
          `Máximo ${MAX_WEEKLY_APPOINTMENTS} citas por semana. Contacte a recepción para excepción`,
        );
      }
 ...
 */
type TransitionRule = 'STAFF' | 'ANY' | 'OWNER';
type StatusKey = 'PENDING' | 'CONFIRMED' | 'COMPLETED' | 'CANCELLED' | 'NO_SHOW';
type TransitionMap = Record<StatusKey, Partial<Record<StatusKey, TransitionRule>>>;
