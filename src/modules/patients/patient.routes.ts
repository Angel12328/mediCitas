// Rutas de perfil del paciente autenticado - mediCitas API
// /patients/me y gestión del contacto de emergencia.
...
async function requirePatientProfile(userId: string): Promise<PatientProfile> {
  ...
  if (!patient) throw new AppError('NOT_FOUND', 'Perfil de paciente no encontrado');
  return {
    id: patient.id,
...
export async function patientProfileRoutes(app: AnyFastifyInstance): Promise<void> {
  ...
  app.patch(
    '/me',
    { preHandler: [...patientOnly, validate({ body: updatePatientSchema })] },
    async (request) => {
  ...
  app.post(
    ...
    { preHandler: [...patientOnly, validate({ body: createEmergencyContactSchema })] },
    async (request, reply) => {
      const existing = await requirePatientProfile(request.user!.id);
  ...
  app.patch(
    '/me/emergency-contact',
    { preHandler: [...patientOnly, validate({ body: updateEmergencyContactSchema })] },
    async (request) => {
      const existing = await requirePatientProfile(request.user!.id);
      if (!existing.emergencyContactName && !existing.emergencyContactNumber) {
