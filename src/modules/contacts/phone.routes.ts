// Rutas de teléfonos vinculados a personas - mediCitas API
// Regla de propiedad: cada usuario gestiona los teléfonos de su propia
// persona; ADMIN gestiona cualquiera.
...
async function authorizePhoneAccess(
  ...
  if (!phone) throw new AppError('NOT_FOUND', 'Teléfono no encontrado');
...
export async function phoneRoutes(app: AnyFastifyInstance): Promise<void> {
  /** Teléfonos de una persona (filtro personId requerido; dueño o ADMIN) */
  app.get(
    '/',
    { preHandler: [authenticate, validate({ query: phonesQuerySchema })] },
    async (request) => {
  ...
  app.post(
    '/',
    { preHandler: [authenticate, validate({ body: createPhoneSchema })] },
    async (request, reply) => {
      const user = request.user!;
      ...
      if (!person) throw new AppError('VALIDATION_ERROR', 'La persona indicada no existe');
  ...
  app.patch(
    ...
    async (request) => {
      ...
      const updates = request.body as Partial<{ number: string; extensionId: string }>;
  ...
  app.delete(
    '/:id',
    { preHandler: [authenticate, validate({ params: phoneIdParamSchema })] },
    async (request, reply) => {
      const user = request.user!;
      const { id } = request.params as { id: string };
