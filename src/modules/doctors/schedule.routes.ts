...
export async function scheduleRoutes(app: AnyFastifyInstance): Promise<void> {
  ...
  app.get(
    ...
    async (request) => {
      const { doctorId, specialtyId, date } = request.query as {
        doctorId: string;
        specialtyId: string;
        date: string;
      };

      const schedules = (await loadDoctorSchedules(doctorId)).filter(
        (s) => s.specialtyId === specialtyId && maskIncludesDay(s.daysBitmask, date)
      );
  ...
  app.patch(
    '/:id',
    { preHandler: [...adminOnly, validate({ params: scheduleIdParamSchema }), validate({ body: updateScheduleSchema })] },
    async (request) => {
