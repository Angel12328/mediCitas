...
async function fetchPage<T>(path: string): Promise<Page<T>> {
  const r = await fetch(appUrl(`/api/proxy${path}`), { cache: "no-store" });
  if (!r.ok) {
...
export function useSchedules(f: {
  doctorId?: string;
  specialtyId?: string;
  status?: string;
  page?: number;
  ...
  qs.set("pageSize", "10");
  return useQuery({
    queryKey: ["schedules", f],
    queryFn: () => fetchPage<ScheduleItem>(`/schedules?${qs}`),
  });
}

export function useAvailability(doctorId?: string, specialtyId?: string, date?: string) {
  return useQuery({
    queryKey: ["availability", doctorId, specialtyId, date],
    enabled: Boolean(doctorId && specialtyId && date),
...
export function useDoctorSchedules(doctorId?: string, specialtyId?: string) {
  return useQuery({
    queryKey: ["doctorSchedules", doctorId, specialtyId],
    enabled: Boolean(doctorId && specialtyId),
    queryFn: async (): Promise<ScheduleItem[]> => {
      const qs = new URLSearchParams({ doctorId: doctorId!, specialtyId: specialtyId! });
      const r = await fetch(appUrl(`/api/proxy/schedules?${qs}`), { cache: "no-store" });
      if (!r.ok) {
        const b = await r.json().catch(() => null) as { title?: string } | null;
        throw new Error(b?.title ?? `Error ${r.status}`);
      }
      const data = await r.json() as Page<ScheduleItem>;
      return data.items;
    },
    staleTime: 1000 * 60 * 5,
  });
...
export function useScheduleAvailability(
  ...
  return useQuery({
    queryKey: ["scheduleAvailability", scheduleId, startDate, endDate],
    enabled: Boolean(scheduleId && startDate && endDate && specialtyId && doctorId),
    queryFn: async (): Promise<BatchAvailabilityResponse> => {
      const qs = new URLSearchParams({
      ...
      return r.json() as Promise<BatchAvailabilityResponse>;
    },
    staleTime: 1000 * 60 * 2,
  });
}
...
export function useCreateSchedule() {
  ...
  return useMutation({
    mutationFn: async (body: unknown) => {
      const r = await fetch(appUrl("/api/proxy/schedules"), {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify(body),
      });
      if (!r.ok) {
        const b = await r.json().catch(() => null) as { detail?: string; title?: string } | null;
        throw new Error(b?.detail ?? b?.title ?? `Error ${r.status}`);
      }
      return r.json();
    },
...
export function useUpdateSchedule() {
  ...
  return useMutation({
    mutationFn: async ({ id, body }: { id: string; body: unknown }) => {
      const r = await fetch(appUrl(`/api/proxy/schedules/${id}`), {
        method: "PATCH",
        headers: { "content-type": "application/json" },
        body: JSON.stringify(body),
      });
...
export function useDeleteSchedule() {
  ...
  return useMutation({
    mutationFn: async (id: string) => {
      const r = await fetch(appUrl(`/api/proxy/schedules/${id}`), { method: "DELETE" });
...
export function bitmaskToDays(mask: number): number[] {
  const out: number[] = [];
