...
import { countryQuerySchema, createCountrySchema, createDepartmentSchema, createMunicipalitySchema, departmentQuerySchema, idParamSchema, treeQuerySchema, } from './location.schemas.js';
...
export async function locationRoutes(app) {
    ...
    app.post('/countries', { preHandler: [...adminOnly, validate({ body: createCountrySchema })] }, async (request, reply) => {
    ...
    app.get('/departments', { preHandler: validate({ query: countryQuerySchema }) }, async (request) => {
        const { countryId } = request.query;
        const params = parseOffsetQuery(request.query);
        const where = { countryId, deletedAt: null };
        const [items, total] = await Promise.all([
    ...
    app.post('/departments', { preHandler: [...adminOnly, validate({ body: createDepartmentSchema })] }, async (request, reply) => {
    ...
    app.get('/municipalities', { preHandler: validate({ query: departmentQuerySchema }) }, async (request) => {
        const { departmentId } = request.query;
        const params = parseOffsetQuery(request.query);
        const where = { departmentId, deletedAt: null };
        const [items, total] = await Promise.all([
    ...
    app.get('/municipalities/:id', { preHandler: validate({ params: idParamSchema }) }, async (request) => {
    ...
    app.post('/municipalities', { preHandler: [...adminOnly, validate({ body: createMunicipalitySchema })] }, async (request, reply) => {
        ...
        try {
            ...
            if (isUniqueViolation(error)) {
                throw new AppError('CONFLICT', 'Ya existe un municipio con ese nombre en el departamento');
            }
            throw error;
        }
    });
